...
根據foodapnda上的店家，首先計算每一期六項人氣精選商品的價格加總，而個別店家當期的CPI上漲率=ln(下一期價格)-ln(當期價格)，再將當期所有店家的CPI上漲率取平均得到當期的CPI上漲率。然而，因為每一期間隔的天數不同，將利用(當期的CPI上漲率/間隔天數)\*365得到年均CPI上漲率，以利各期之間的比較。
...
下表為各期所選的樣本店家區域類別的占比。有別於母體台式及無區域兩類占比相當，每期所篩選出的樣本以台式占多數，每期幾乎皆有20%以上的差距。將根據店家的類別進行加權處理，以確保每一種類型的的店家樣本能夠反映母體的區域類型結構。
...
根據2023年8月6日的營業時段母體分配，午晚餐占59.65%、晚餐占16.46%、早午晚餐占11.84%、早午餐占10.01%、午餐占0.75%、早餐占0.61%、其他占0.36%、早晚餐占0.32%。

<img src="image/population_businessHour.png" width="1000" />

下表為各期所選的樣本店家營業時段的占比。每一期將皆以午晚餐為主要類別，皆有著接近60%的占比。整體而言，所有類別的占比和母體差距不大。
...
比較未經加權的CPI上漲率及經營業時段加權的CPI上漲率，經營業時段加權的CPI上漲率普遍略低於未經加權的CPI上漲率。特別是在2023年5月31日的差異較為明顯，差距來到0.7532%。
